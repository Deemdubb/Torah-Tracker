  ...
  return (
    <div className="min-h-dvh bg-background text-foreground" dir={dir}>
      ...
      <main className="max-w-3xl mx-auto px-4 pt-3 safe-bottom">
        ...
        {error && (
          <div className="mb-3"><Banner tone="error"><div className="flex items-center gap-2 flex-wrap"><span className="flex-1">{error === 'db-needs-update' ? t('dbNeedsUpdate') : `${t('error')}: ${error}`} <span className="opacity-60 text-xs" dir="ltr">({__APP_VERSION__})</span></span><Button size="sm" variant="outline" onClick={() => { setError(null); reload({}); }}>{t('retry')}</Button></div></Banner></div>
        )}
