...
export default function ProgressPill({ name, path, total, completed, subtitle }) {
  const { dir } = useLang();
  ...
  return (
    <Link to={path} className="tap group flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-card border border-border hover:border-primary/40 active:bg-card/70 transition-all">
      <div className="flex-1 text-start min-w-0">
        <div className="text-lg font-display font-semibold leading-tight truncate"><Name>{name}</Name></div>
